...
from models import NMP_QEModel
...
def parse_args(args=None):
    ...
    parser.add_argument('--geo', default='gmm', type=str, choices=['vec', 'gaussian', 'beta', 'gmm'], help='the reasoning model, vec for GQE, gaussian for PERM, beta for BetaE, gmm for NMPQEM')
    parser.add_argument('--print_on_screen', action='store_true', default=True)
    parser.add_argument('--tasks', default='1p.2p.3p.2i.3i.ip.pi.2u.up', type=str, help="tasks connected by dot, refer to the BetaE paper for detailed meaning and structure of each task") # 2in.3in.inp.pin.pni.
    parser.add_argument('--seed', default=0, type=int, help="random seed")
    parser.add_argument('-gmm', '--gmm_mode', default="(none, 10, 1)", type=str, help='(activation, gmm_num) for GMM')
...
def save_best_model(model, optimizer, save_variable_list, args):
    '''
    Save the parameters of the model and the optimizer,
    as well as some other variables such as step and learning_rate
    '''
...
def evaluate_best_model(model, tp_answers, fn_answers, args, dataloader, query_name_dict, mode, step, writer):
    '''
    Evaluate queries in dataloader
    '''
    average_metrics = defaultdict(float)
...
def load_data(args, tasks):
    ...
    # del train_queries[name_query_dict['2p']]
    # del train_queries[name_query_dict['3p']]
    # del train_queries[name_query_dict['3i']]
    # del train_queries[name_query_dict['3in']]
    # del train_queries[name_query_dict['pni']]
    # del train_queries[name_query_dict['pin']]
    # pdb.set_trace()
...
def main(args):
    ...
    for task in tasks:
        if 'n' in task and args.geo in ['vec']:
            assert False, "Gmm cannot handle queries with negation"
    ...
    if args.geo in ['gmm']:
        tmp_str = "g-{}-mode-{}".format(args.gamma, args.gmm_mode)
    ...
    logging.info('experiment information: %s' % args.exp_info)
    ...
    for name, param in model.named_parameters():
        logging.info('Parameter %s: %s, require_grad = %s, %s' % (name, str(param.size()), str(param.requires_grad), str(param.is_leaf)))
    ...
    if args.checkpoint_path is not None:
        ...
        checkpoint = torch.load(os.path.join(args.checkpoint_path, 'best_checkpoint'))
    ...
    step = init_step
    if args.geo == 'gmm':
        logging.info('gaussian mode = %s' % args.gmm_mode)
    elif args.geo == 'beta':
    ...
    if args.do_train:
        best_avg_metrics = defaultdict(float)
        best_mrr = 0.
        best_metrics = defaultdict(lambda: defaultdict(int))
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step % args.valid_steps == 0 and step > 0:
                if args.do_valid:
                    ...
                    valid_all_metrics, average_metrics = evaluate(model, valid_easy_answers, valid_hard_answers, args, valid_dataloader, query_name_dict, 'Valid', step, writer)

                if args.do_test:
                    logging.info('Evaluating on Test Dataset...')
                    test_all_metrics, average_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_dataloader, query_name_dict, 'Test', step, writer)
                    # pdb.set_trace()
                    avg_mrr = average_metrics['MRR']
                    avg_h10 = average_metrics['HITS10']
                    avg_h3 =  average_metrics['HITS3']
                    avg_h1 = average_metrics['HITS1']
                    
                    if avg_mrr > best_mrr:
                        save_best_model(model, optimizer, save_variable_list, args)
                        best_mrr = avg_mrr

                    log_metrics('%s average'%'Best_Test', step, average_metrics)
                    
                    for query_structure in test_all_metrics.keys():
                        for metric in test_all_metrics[query_structure].keys():
                            if metric == 'num_queries':
                                continue
                            else:       
                                if test_all_metrics[query_structure][metric] > best_metrics[query_structure][metric]:
                                    best_metrics[query_structure][metric] = test_all_metrics[query_structure][metric]

                    for query_structure in best_metrics.keys():
                        log_metrics('Best Test'+" "+query_name_dict[query_structure], step, best_metrics[query_structure])
    ...
    if args.do_test:
        logging.info('Evaluating on Test Dataset...')
        test_all_metrics, test_average_metrics = evaluate(model, test_easy_answers, test_hard_answers, args, test_dataloader, query_name_dict, 'Test', step, writer)
